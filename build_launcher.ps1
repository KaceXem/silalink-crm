    ...
    public class MainWindow : Window
    {
        private ServiceConfig[] services = new ServiceConfig[]
        {
            new ServiceConfig {
                ...
                Description = "Moteur d'inférence LLM B2B (Port 20128)",
                Port = 20128,
        ...
        public MainWindow()
        {
            Title = "SilaLink CRM — Control Hub";
            Width = 520;
            Height = 620;
        ...
        private void BuildUI()
        {
            ...
            {
                Text = "Superviseur de déploiement et d'orchestration locale",
                FontSize = 12,
            ...
            {
                Text = "Démarrage des sous-systèmes en tâche de fond...",
                FontSize = 11,
                Foreground = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#94A3B8")),
            ...
            {
                Content = "Arrêter tous les services et quitter",
                Height = 32,
                FontSize = 11,
        ...
        private async Task StartOrchestrationAsync()
        {
            ...
            {
                var svc = services[i];
                globalStatusText.Text = string.Format("Démarrage du service : {0}...", svc.Name);
                ...
                if (isUp)
                {
                    ...
                    statusTexts[i].Text = "OPÉRATIONNEL";
                ...
                else
                {
                    ...
                    globalStatusText.Text = string.Format("Échec de connexion sur le port {0}.", svc.Port);
                    return;
                }
            }

            // Prêt
            globalStatusText.Text = "Tous les micro-services sont actifs et prêts.";
            btnLaunchApp.IsEnabled = true;
            btnLaunchApp.Content = "🚀 Ouvrir SilaLink CRM";
...
$csc = Join-Path $netFrameworkPath "csc.exe"

if (!(Test-Path $csc)) {
    Write-Error "Compilateur C# non trouvé dans $netFrameworkPath"
    exit
}

$outputExe = "C:\Users\USER\Desktop\prototype B2B - Modernisation\SilaLinkHub.exe"
$sourceFile = "C:\Users\USER\Desktop\prototype B2B - Modernisation\LauncherSource.cs"

[System.IO.File]::WriteAllText($sourceFile, $source, [System.Text.Encoding]::UTF8)

Write-Host "Compilation de l'exécutable WPF en cours..." -ForegroundColor Cyan

& $csc /target:winexe /out:$outputExe /r:System.dll /r:System.Core.dll /r:PresentationCore.dll /r:PresentationFramework.dll /r:WindowsBase.dll /r:System.Xaml.dll $sourceFile

if (Test-Path $outputExe) {
    Remove-Item $sourceFile -Force
    Write-Host "SUCCÈS : SilaLinkHub.exe généré avec succès !" -ForegroundColor Green
} else {
